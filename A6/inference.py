...
class ParticleFilter(InferenceModule):
	...
	def observe(self, observation, gameState):
		...
		if noisyDistance == None:
			...
			weightedBelief = util.Counter()
			for p in belief.keys():
				trueDistance = util.manhattanDistance(p, pacmanPosition)
				if emissionModel[trueDistance] > 0:
					weightedBelief[p] = emissionModel[trueDistance]*belief[p]
			if not any(weightedBelief) or weightedBelief.totalCount() == 0:
				self.initializeUniformly(gameState)
			else:
				weightedBelief.normalize()
				self.particles = []
				for i in range(self.numParticles):
					self.particles.append(util.sample(weightedBelief))
...
class JointParticleFilter:
	...
	def initializeParticles(self):
		...
		"""
		"*** YOUR CODE HERE ***"
		listGhostPos = [self.legalPositions]*self.numGhosts
		legalStates = list(itertools.product(*listGhostPos))
		random.shuffle(legalStates)
		self.particles = []
		n = len(legalStates)
		for i in range(self.numParticles):
			self.particles.append(legalStates[i%n])
